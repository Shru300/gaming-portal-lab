# gaming-portal-lab
# 🎮 Cloud Gaming Portal

This repository contains the source code for **Activity 01**, a cloud-hosted gaming website.

## 📋 Student Details
* **Name:** Shruti Rathi
* **Registration No:** 23BCG10099
* **Course:** CSE (Gaming Technology)
* **Subject:** Gaming in Cloud
* **University:** VIT Bhopal University

## 🚀 Live Demo
You can view the live website here:  
👉 **[Click to Open Gaming Portal](https://shru300.github.io/gaming-portal-lab/)** 
## 🛠️ Project Objectives
1.  **Objective 1:** Created a Home Page for the gaming portal and hosted it using GitHub Pages.
2.  **Objective 2:** Created a GitHub repository and shared access with collaborators.
3.  **Objective 3:** Developed a "Number Guessing Game" and hosted it on a cloud platform (GitHub Pages).

## 📂 Files Included
* `index.html`: The main landing page for the portal.
* `game.html`: The logic for the web-based game.

---
*Submitted for Activity 01 - Gaming in Cloud.*
